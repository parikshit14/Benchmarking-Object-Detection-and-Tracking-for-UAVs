...
sys.path.append('../trackers/ByteTrack/')
...
def main():
    ...
    for frame in mmcv.track_iter_progress(cap):
        ...
        if bbox_result is not None:
            ...
            online_im = show_result

        out.write(online_im)
        if args.show:
            cv2.imshow('Bytetrack Tracking with CEASC', online_im)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
        
        # output is not coming check plot tracking function
        
        #with open("tracker.txt", 'w') as f:
        #    f.writelines(results_list)
        #print("save results to tracker.txt")
    
    avg_fps = total_fps / frame_count
    print(f"Average FPS: {avg_fps:.3f}")
    
    avg_fps_tracker = total_fps_tracker / frame_count
    print(f"Average Tracker FPS: {avg_fps_tracker:.3f}")
