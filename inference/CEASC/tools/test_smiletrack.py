...
def main():
    ...
    tracker = SMILEtrack(args, frame_rate=30.0)
    #timer=Timer()
    results_list = []
    ...
    for frame in mmcv.track_iter_progress(cap):
        ...
        if bbox_result is not None:
            ...
            online_im = plot_tracking(
                    frame, online_tlwhs, online_ids, online_cls, cls_names, frame_id=frame_count, fps=total_comb_fps / frame_count
                )
        else:
        ...
        out.write(online_im)
        if args.show:
            cv2.imshow('SMILEtrack Tracking with CEASC', online_im)
